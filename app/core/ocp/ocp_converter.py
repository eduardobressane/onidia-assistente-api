...
class OCPConverter:
    ...
    @staticmethod
    def _ocpm_to_ocp_v1_0_0(ocpm_obj: Dict, url: str = "", headers: Optional[List] = None) -> Dict:
        return {
            "metadata": {
                "protocol": "OCP",
                "version": "1.0.0",
            ...
            "structure": {
                "description": ocpm_obj.get("metadata", {}).get("description", ""),
                "input_schema": ocpm_obj.get("input_schema", {}),
                "output_schema": ocpm_obj.get("output_schema", {}),
                "tools": [
                    {
                        "name": tool.get("name"),
                        "description": tool.get("description", ""),
                    ...
                    for tool in ocpm_obj.get("tools", [])
                ]
            }
        }
    ...
    # =========================
    def ocp(ocpType: Literal["mcp", "ocp-m", "langserve"], obj: Dict, url: str = "", headers: Optional[Dict[str, Any]] = None, version: str = "1.0.0"):
        if "mcp" in ocpType:
            return OCPConverter.mcp_to_ocp(mcp_obj=obj, url=url, headers=headers, version=version)
        elif "ocp-m":
            return OCPConverter.ocpm_to_ocp(ocpm_obj=obj, url=url, headers=headers, version=version)
        else:
    ...
    @staticmethod
    def ocpm_to_ocp(ocpm_obj: Dict, url: str = "", headers: Optional[List] = None, version: str = "1.0.0") -> Dict:
        """Converte um objeto MCP para OCP na versão especificada."""
        ...
        if version == "1.0.0":
            return OCPConverter._mcp_to_ocp_v1_0_0(ocpm_obj, url, headers)
        else:
            raise BadRequestError(f"Versão OCM-M->OCP não suportada: {version}")
