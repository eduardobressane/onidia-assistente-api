...
from app.core.security import require_permissions, get_current_user, validate_contractor_access
...
def get_all(ai_model_id: str, contractor_id: Optional[UUID] = Query(None), current_user: dict = Depends(get_current_user)):
    # If contractor_id is not informed, use the cid of the current user
    if contractor_id is None:
        contractor_id = current_user.get("cid")
    else:
        validate_contractor_access(current_user, contractor_id)
...
def create(
    ...
    # If contractor_id is not provided, use the current user's cid
    if contractor_id is None:
        contractor_id = current_user.get("cid")
    else:
        validate_contractor_access(current_user, contractor_id)
