...
def listar(
    ...
    agentes: List[AgenteOutList] = AgenteService.listar(id_contratante, nome, page, rpp)
    return ok(total=agentes["total"], pages=agentes["pages"], data=agentes["items"])


@router.get("/{id}", response_model=AgenteOutDetail, dependencies=[Depends(require_permissions(["*", "hafj0kaclm"]))])
def obter(id: str):
    agente: AgenteOutInterno = AgenteService.obter(id)
    
    return AgenteOutDetail(**agente.dict())


@router.post("", response_model=HttpResponse[AgenteOutDetail], dependencies=[Depends(require_permissions(["*", "hafj0qu4kb"]))])
def criar(payload: AgenteCreate, id_contratante: Optional[UUID] = Query(None), current_user: dict = Depends(get_current_user)):
    # Se id_contratante não for informado, usar o cid do usuário atual
    if id_contratante is None:
        id_contratante = current_user.get("cid")
    else:
...
def atualizar(id: str, payload: AgenteUpdate, current_user: dict = Depends(get_current_user)):
    agente: AgenteOutInterno = AgenteService.obter(id)
