...
async def listar(id_tool: str, id_contratante: Optional[UUID] = Query(None), current_user: dict = Depends(get_current_user)):
    ...
    rows: List[CredencialToolOutList] = await CredencialToolService.listar(id_tool, id_contratante)
...
async def obter(id_tool: str, id: str, current_user: dict = Depends(get_current_user)):
    credencial: CredencialToolInterna = await CredencialToolService.obter(id)
...
async def criar(
    ...
    novo: CredencialToolOutDetail = await CredencialToolService.criar(
...
async def atualizar(id_tool: str, id: str, payload: CredencialToolUpdate, current_user: dict = Depends(get_current_user)):
    credencial: CredencialToolInterna = await CredencialToolService.obter(id)
    ...
    atualizado: CredencialToolOutDetail = await CredencialToolService.atualizar(id, payload)
...
async def remover(id_tool: str, id: str, current_user: dict = Depends(get_current_user)):
    credencial: CredencialToolInterna = await CredencialToolService.obter(id)
