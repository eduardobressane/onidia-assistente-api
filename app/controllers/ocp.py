...
@router.get("", response_model=HttpResponse[List[OCPOutList]], dependencies=[Depends(require_permissions(["*", "hafir5fmxh"]))])
def get_all(
...
@router.get("/{id}", response_model=OCPOutDetail, dependencies=[Depends(require_permissions(["*", "hafirf720i"]))])
def get_by_id(id: str):
    ocp: OCPOutDetail = OCPService.get_by_id(id)
    return ocp


@router.post("", response_model=HttpResponse[OCPOutDetail], dependencies=[Depends(require_permissions(["*", "hafirojaiq"]))])
def create(
    payload: OCPCreate,
    contractor_id: Optional[UUID] = Query(None),
...
@router.put("/{id}", response_model=HttpResponse[OCPOutDetail], dependencies=[Depends(require_permissions(["*", "hafis7dqeb"]))])
